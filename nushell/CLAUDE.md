...
- **`scripts/`** — Domain-specific scripts, sourced via `source <name>.nu` in config.nu. Resolved through the default `$NU_LIB_DIRS` (includes `scripts/` in the config dir).

## Conventions

- **Cross-platform** (Windows, Linux, macOS, Android): branch with the `is-windows` / `is-android` / `is-macos` helpers from `env.nu`, never raw `$nu.os-info.*`.
- **Wrapped commands**: CLI wrappers use `def --wrapped` to pass unknown flags to the wrapped tool.
- **Env-changing commands**: `def --env` when the command runs `cd` or sets env vars.
...
- **Minimal config**: Only non-default `$env.config` values. Check defaults with `nu -n -c '$env.config.<field>'` or `config nu --doc`.
